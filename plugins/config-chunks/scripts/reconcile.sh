...
# The size gate is fail-CLOSED: an over-cap chunk aborts the reconcile and the
# previous bundle survives untouched. A warning that ships anyway trains every
# reader to ignore it, which is how the caps came to be routinely breached.
# Two sanctioned escapes: per-chunk `oversize_approved:` frontmatter (durable,
# re-announced every run) or --allow-oversize / CONFIG_CHUNKS_ALLOW_OVERSIZE=1
# (this run only).
ALLOW_OVERSIZE="${CONFIG_CHUNKS_ALLOW_OVERSIZE:-0}"
for arg in "$@"; do
  case "$arg" in
    --allow-oversize) ALLOW_OVERSIZE=1 ;;
    *) echo "${OWNER}: unknown argument '$arg'" >&2; exit 2 ;;
  esac
done
...
# --- size gate (PRE-FLIGHT — must precede assembly) ---
# Runs before a single byte of the bundle is written, so a violation leaves the
# previous bundle.md intact rather than truncating it mid-stream. A chunk may
# carry `oversize_approved: <reason>` in its frontmatter to opt out; the reason
# is echoed on every reconcile so an exemption stays visible instead of silently
# becoming permanent. `--allow-oversize` downgrades the whole gate to warnings.
gate_failures=0
while IFS=$'\t' read -r name ver ord file; do
  [ -n "$name" ] || continue
  disclosure=$(fm_value disclosure "$file")
  [ -n "$disclosure" ] || disclosure="inline"
  if [ "$disclosure" = "pointer" ]; then cap=$POINTER_SOFT_CAP; else cap=$INLINE_SOFT_CAP; fi

  body_bytes=$(strip_frontmatter "$file" | wc -c | tr -d ' ')
  [ "$body_bytes" -gt "$cap" ] || continue

  approved=$(fm_value oversize_approved "$file")
  if [ -n "$approved" ]; then
    echo "${OWNER}: oversize-approved: '$name' is ${body_bytes} chars (cap: ${cap}) — ${approved}" >&2
  elif [ "$ALLOW_OVERSIZE" = "1" ]; then
    echo "${OWNER}: WARNING: chunk '$name' body is ${body_bytes} chars (cap: ${cap}, disclosure: ${disclosure}) — shipped anyway via --allow-oversize." >&2
  else
    echo "${OWNER}: ERROR: chunk '$name' body is ${body_bytes} chars (cap: ${cap}, disclosure: ${disclosure})." >&2
    echo "${OWNER}:        Trim it, split it, or convert to 'disclosure: pointer' + skill." >&2
    echo "${OWNER}:        Deliberate exception? Add 'oversize_approved: <reason>' to its frontmatter." >&2
    gate_failures=$((gate_failures + 1))
  fi
done < "$tmp/ordered"

if [ "$gate_failures" -gt 0 ]; then
  echo "${OWNER}: ABORTED — ${gate_failures} chunk(s) over cap. bundle.md left unchanged." >&2
  echo "${OWNER}:        Override for this run with: reconcile.sh --allow-oversize" >&2
  exit 1
fi
...
# --- assemble the bundle (this plugin owns bundle.md wholesale) ---
{
  ...
  while IFS=$'\t' read -r name ver ord file; do
    ...
    rm -f "$body_file"
    echo
    # Size enforcement happens in the pre-flight gate above, not here — a check
    # inside this redirect can only fire after bytes are already committed.
  done < "$tmp/ordered"
} > "$BUNDLE"
