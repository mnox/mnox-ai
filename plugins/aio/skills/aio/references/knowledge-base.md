...
**Evidence:** ACE — Agentic Context Engineering (arxiv 2510.04618, ICLR 2026): contexts as evolving playbooks via Generator/Reflector/Curator roles, prevents brevity bias and context collapse, +10.6% on agent benchmarks. Less Context, Better Agents (arxiv 2606.10209, Jun 2026): on a 50-task enterprise tool-use benchmark, pruning to the last 5 tool-call pairs + summarization hits 91.6% completion at ~553K tokens vs 71.0% at ~1.48M tokens for full history — just-in-time context strictly dominates accumulate-everything (GPT-5, Sonnet 4.5). "Effective Context Engineering for AI Agents" (Anthropic Engineering, 2025): sub-agent isolation, compaction, structured memory over raw conversation stuffing.
...
**Evidence:** A-MEM (arxiv 2502.12110, Xu et al., 2025): Zettelkasten-style structured memory with dynamic indexing and cross-linking outperforms flat retrieval stores. MemGPT (arxiv 2310.08560, Packer et al., 2023, UC Berkeley): OS-inspired virtual context management, tiered main+disk memory enables beyond-context-window operation. Graph-based Agent Memory survey (arxiv 2602.05665): graph retrieval (relationship traversal) enables multi-hop reasoning flat vector stores miss; open problems — identity resolution, staleness detection. Memory for Autonomous LLM Agents survey (arxiv 2603.07670): temporal-scope × representational-substrate × control-policy taxonomy; move eval from static recall to multi-session agentic tests. Cross-Scenario Generality (arxiv 2606.04315, Jun 2026): across 8 memory systems × 5 scenarios the dominant lever is agent-controlled read/write (tool-calls over even flat text-file storage), not storage topology — a plain agent-driven baseline (AutoMEM) beats elaborate architectures cross-scenario; structure helps only once the agent controls access.
...
**Evidence:** PROBE (arxiv 2605.08717, May 2026): three-layer telemetry→diagnosis→bounded-guidance framework hits 65.4% Top-1 diagnosis accuracy on 257 unresolved SWE cases but only 21.8% recovery success — the diagnosis–recovery gap. CausalFlow (arxiv 2605.25338, May 2026): step-level Causal Responsibility Scores via counterfactual intervention + minimally-edited repairs convert 42.7% of failed executions into validated (wrong-step, corrected-step) pairs across 3,000+ problems — quantifies turning attribution into actionable recovery. ACRFence (arxiv 2603.20625): checkpoint-restore recovery is itself a hazard — agents re-synthesize subtly different requests after restore ("semantic rollback attacks": duplicate payments, credential reuse); record irreversible effects and enforce replay-or-fork semantics.
...
### [KB:judge-validity] Validate your LLM-as-judge — high consistency is not validity
**Rule:** Before trusting an LLM-as-judge eval layer, audit it for agreement, consistency, and bias; a judge can be highly self-consistent yet invalid (position/verbosity bias, low human agreement). Never read judge test-retest stability as correctness, and re-audit judges per benchmark and per language.
**Evidence:** Reliability without Validity (arxiv 2606.19544, Jun 2026): 21 judges × 9 providers × ~541K judgments — judges reach >0.95 test-retest yet Cohen's κ drops 33–41pp below raw agreement, rankings swing up to 14 positions across benchmarks, production judges show severe position bias (>0.10); prescribes a Minimum Viable Validation Protocol. BabelJudge (arxiv 2606.22329, Jun 2026): label-free audit of four judge failure modes (position/verbosity bias, order inconsistency, cross-lingual degradation) — sharp degradation in lower-resource languages and over agent trajectories.
...
**Evidence:** OWASP Top 10 for Agentic Applications (Dec 2025): industry-standard, peer-reviewed by 100+ experts; 10 Agentic Security Issues (Goal Hijack, Memory Poisoning, Tool Misuse, Inter-Agent Comms); "least agency" as governing principle. AgenticRei (arxiv 2606.19464, Jun 2026): production policy engines (XACML/Rego/Cedar) express only permit/prohibit — runtime governance also needs obligation-lifecycle management, dispensations, and meta-policy conflict resolution, modeled as an external OWL/Rei deontic policy enforced outside the LLM over both tool calls and agent-to-agent messages. Microsoft Agent Governance Toolkit (Apr 2026): open-source 7-package toolkit (Python/TS/Rust/Go/.NET), zero-trust identity + sandboxing + policy + SRE-for-agents at sub-ms latency; first to cover all 10 OWASP Agentic risks; integrates LangChain/CrewAI/Google ADK/OpenAI Agents SDK. "Agent control plane" pattern: governance outside the loop for independent oversight; most teams layer NeMo (input/flow) + Guardrails AI (output).
...
**Evidence:** Semantic Kernel CVEs (Microsoft Security, May 7 2026): CVE-2026-26030 + CVE-2026-25592 — unsafe string interpolation in filter functions + exposed file-write tools turn prompt injection into arbitrary code execution. vm2 Sandbox-Escape CVE Wave (Kodem Security, May 14 2026): 13 CVEs CVSS 9.0–10.0 chain prompt injection → JS-sandbox escape → host RCE; mitigation generalizes — process-level isolation beneath the language sandbox. DuneSlide (Cato Networks, CVE-2026-50548/50549, Jun 2026): two 9.8-CVSS zero-click RCEs in Cursor IDE — injection from an MCP server or poisoned web result steers the agent's terminal working-directory out of scope or abuses symlink canonicalization to overwrite the sandbox binary itself; the sandbox boundary is the payload's target, not just the tool.
...
**Evidence:** Calibrated Claim-Level Specificity (arxiv 2604.17487, Apr 2026): localized semantic backoff at claim granularity — emit *less specific* claims when confidence drops on sub-parts, instead of binary answer/refuse; preserves utility where global refusal tanks it. UQ for Computer-Use Agents (arxiv 2606.25760, Jun 2026): calibration is NOT portable — UQ rankings hold within a model but degrade sharply across model classes or UI mismatch and vendor confidence transfers poorly; re-calibrate any confidence gate on the target system, never inherit it. TRUST (arxiv 2606.06976, Jun 2026): outcome-reward RL collapses the correct-vs-incorrect confidence gap; an uncertainty-repulsion reward term preserves calibrated separation so agents reliably decide invoke-vs-abstain-vs-answer. (Confidence gating: see [KB:hitl]. Guaranteed recovery path: see [KB:diagnosis-recovery-gap].)
...
**Evidence:** Don't Break the Cache (arxiv 2601.06007): first cross-provider eval of prompt caching for long-horizon agents — strategic caching cuts API cost 41–80% and TTFT 13–31%, but naive full-context caching can paradoxically *increase* latency. Leyline (arxiv 2606.01065, Jun 2026): application-level KV-cache directives let the agent tell the engine which cache computations to retain/discard/reuse across steps — fine-grained cache-lifecycle control cuts recomputation in long agent loops instead of treating cache as opaque.
...
**Evidence:** Routing/Cascades game theory (arxiv 2602.09902, Feb 2026): Stackelberg game between cost-minimizing providers and utility-maximizing users (who re-prompt or abandon) — optimal policy is often a static threshold with no cascading; cascading can degrade outcomes under incentive misalignment. Cluster-Route-Escalate (arxiv 2606.27457, Jun 2026): cluster queries → assign each cluster its most cost-effective model (one interpretable knob) → escalate low-confidence outputs via quality estimation; hits 97–99% of the strongest model's accuracy at lower time-per-output-token using only correctness labels, adapting to pool changes without reconfig.
...
**Evidence:** TRACE (arxiv 2604.05336, Apr 2026): contrasts successful vs failed trajectories to diagnose missing capabilities, synthesizes targeted RL environments per gap, trains LoRA adapters, routes at inference — +14.1 pts τ²-bench, beats GRPO and GEPA on scaling efficiency. ACE evolving playbooks (arxiv 2510.04618, ICLR 2026): see [KB:context-eng]. DSPy / GEPA (Stanford NLP): typed signatures + optimizer auto-find prompts (ReAct 24%→51% HotPotQA, transfers across models); GEPA (Genetic-Pareto) supersedes MIPROv2 — trajectory sampling + NL reflection + Pareto frontier, in OpenAI's self-evolving agent cookbook. MAS-PromptBench (arxiv 2606.23664, Jun 2026): across 4 frameworks × 9 tasks × 5 topologies, programmatic prompt optimization delivers large gains from weak/minimal prompts but sharply diminishing returns once a prompt already encodes task structure — a boundary condition for when PO is worth it before escalating to training.
